# Analyse Lexicographique du Kreyòl Guadeloupéen

## Métadonnées du Corpus

- **Date de génération** : 27 August 2026 à 17:34
- **Version du pipeline** : 3.0 - Pipeline Unique
- **Source des données** : Dataset POTOMITAN/PawolKreyol-gfc (Hugging Face)
- **Nombre de textes** : 2531
- **Tokens totaux** : 36,501
- **Types lexicaux** : 5,305

---

## 1. Corpus et Échantillonnage

### 1.1 Taille et Couverture

- **Total des tokens** : 36,542
- **Types lexicaux uniques** : 5,305
- **Type-Token Ratio (TTR)** : 0.1452
- **Richesse lexicale** : Élevée

## 2. Analyse Morphologique

### 2.1 Distribution par Longueur

| Longueur | Nombre de mots | Pourcentage |
|----------|----------------|-------------|
|  2 lettres |    102 |   1.9%  |
|  3 lettres |    418 |   7.9% ███ |
|  4 lettres |    730 |  13.8% ██████ |
|  5 lettres |    925 |  17.4% ████████ |
|  6 lettres |    933 |  17.6% ████████ |
|  7 lettres |    722 |  13.6% ██████ |
|  8 lettres |    530 |  10.0% ████ |
|  9 lettres |    373 |   7.0% ███ |
| 10 lettres |    232 |   4.4% ██ |
| 11 lettres |    129 |   2.4% █ |
| 12 lettres |     89 |   1.7%  |
| 13 lettres |     72 |   1.4%  |
| 14 lettres |     23 |   0.4%  |
| 15 lettres |     13 |   0.2%  |
| 16 lettres |      6 |   0.1%  |
| 17 lettres |      4 |   0.1%  |
| 18 lettres |      1 |   0.0%  |
| 19 lettres |      1 |   0.0%  |
| 21 lettres |      1 |   0.0%  |
| 22 lettres |      1 |   0.0%  |

### 2.2 Mots Composés (avec trait d'union)

- **Total** : 1090 mots (20.5%)
- **Exemples** : a-y, an-mwen, a-w, an-nou, a-yo, ba-w, fi-la, ban-mwen, fè-mwen, kaz-la, rann-nou, di-mwen, ba-y, péyi-la, moun-la

## 3. Analyse Phonographématique

### 3.1 Caractères Diacritiques

| Caractère | Fréquence | Usage |
|-----------|-----------|-------|
| **é** | 2,032 | Très fréquent
| **è** | 1,166 | Très fréquent
| **ò** | 437 | Très fréquent
| **à** | 12 | Modéré
| **ô** | 2 | Rare |
| **ê** | 1 | Rare |

### 3.2 Digrammes les Plus Fréquents

| Digramme | Fréquence |
|----------|-----------|
| **an** | 1,201 |
| **la** | 888 |
| **ou** | 775 |
| **on** | 597 |
| **-l** | 504 |
| **en** | 496 |
| **ma** | 390 |
| **ko** | 339 |
| **ch** | 324 |
| **ra** | 318 |
| **nn** | 315 |
| **al** | 293 |
| **as** | 285 |
| **é-** | 267 |
| **ka** | 265 |
| **yé** | 256 |
| **ri** | 252 |
| **ré** | 252 |
| **n-** | 238 |
| **ti** | 231 |

## 4. Analyse Lexicale Stratifiée

### 4.1 Distribution de Fréquence (Loi de Zipf)

- **Hapax legomena** (freq=1) : 2,966 mots (55.9%)
- **Dis legomena** (freq=2) : 796 mots (15.0%)
- **Mots rares** (freq 3-5) : 774 mots
- **Mots fréquents** (freq 6-20) : 542 mots
- **Mots très fréquents** (freq >20) : 227 mots

### 4.2 Principe de Pareto

- **794 mots** (15.0%) représentent **80%** des occurrences
- **Vocabulaire fondamental** : Les 1000 mots les plus fréquents

### 4.3 Vocabulaire Fondamental (Top 50)

| Rang | Mot | Fréquence | % Cumul |
|------|-----|-----------|---------|
|  1 | **ka** | 1,800 | 4.93% |
|  2 | **an** | 1,388 | 8.72% |
|  3 | **sé** | 885 | 11.15% |
|  4 | **on** | 833 | 13.43% |
|  5 | **pou** | 727 | 15.42% |
|  6 | **té** | 726 | 17.40% |
|  7 | **pa** | 664 | 19.22% |
|  8 | **yo** | 638 | 20.96% |
|  9 | **nou** | 633 | 22.70% |
| 10 | **ou** | 611 | 24.37% |
| 11 | **ki** | 548 | 25.87% |
| 12 | **la** | 481 | 27.19% |
| 13 | **ni** | 408 | 28.30% |
| 14 | **fè** | 404 | 29.41% |
| 15 | **sa** | 401 | 30.50% |
| 16 | **mwen** | 400 | 31.60% |
| 17 | **ké** | 337 | 32.52% |
| 18 | **moun** | 267 | 33.25% |
| 19 | **di** | 251 | 33.94% |
| 20 | **pé** | 244 | 34.61% |
| 21 | **si** | 218 | 35.20% |
| 22 | **kon** | 214 | 35.79% |
| 23 | **lè** | 211 | 36.37% |
| 24 | **tout** | 206 | 36.93% |
| 25 | **adan** | 202 | 37.48% |
| 26 | **mé** | 187 | 37.99% |
| 27 | **vwè** | 165 | 38.45% |
| 28 | **pran** | 152 | 38.86% |
| 29 | **zòt** | 145 | 39.26% |
| 30 | **menm** | 144 | 39.65% |
| 31 | **ti** | 130 | 40.01% |
| 32 | **a-y** | 130 | 40.36% |
| 33 | **krab** | 129 | 40.72% |
| 34 | **kou** | 125 | 41.06% |
| 35 | **ay** | 113 | 41.37% |
| 36 | **dé** | 111 | 41.67% |
| 37 | **èvè** | 111 | 41.98% |
| 38 | **ès** | 110 | 42.28% |
| 39 | **asi** | 109 | 42.58% |
| 40 | **bon** | 98 | 42.84% |
| 41 | **jan** | 98 | 43.11% |
| 42 | **épi** | 97 | 43.38% |
| 43 | **ja** | 97 | 43.64% |
| 44 | **rivé** | 95 | 43.90% |
| 45 | **viktò** | 95 | 44.16% |
| 46 | **ba** | 92 | 44.41% |
| 47 | **byen** | 91 | 44.66% |
| 48 | **tini** | 91 | 44.91% |
| 49 | **dlo** | 90 | 45.16% |
| 50 | **pè** | 90 | 45.41% |

## 5. Analyse Syntaxique et Collocations

### 5.1 Bigrammes les Plus Fréquents

| Rang | Bigramme | Fréquence |
|------|----------|-----------|
|  1 | **té ka** | 184 |
|  2 | **ka fè** | 161 |
|  3 | **ou ka** | 145 |
|  4 | **an ka** | 142 |
|  5 | **yo ka** | 116 |
|  6 | **té ké** | 111 |
|  7 | **an mwen** | 106 |
|  8 | **an té** | 99 |
|  9 | **ni on** | 91 |
| 10 | **pa ka** | 87 |
| 11 | **an nou** | 82 |
| 12 | **sé on** | 81 |
| 13 | **nou ka** | 73 |
| 14 | **pa té** | 68 |
| 15 | **ka di** | 66 |
| 16 | **pa ni** | 63 |
| 17 | **ki té** | 60 |
| 18 | **ka pran** | 59 |
| 19 | **ka vwè** | 59 |
| 20 | **pè pyè** | 59 |
| 21 | **adan on** | 55 |
| 22 | **ou pé** | 54 |
| 23 | **ou pa** | 53 |
| 24 | **la ka** | 52 |
| 25 | **ou té** | 51 |
| 26 | **moun ka** | 50 |
| 27 | **yo té** | 50 |
| 28 | **pou nou** | 49 |
| 29 | **té ni** | 47 |
| 30 | **ès ou** | 47 |

### 5.2 Marqueurs Temps-Mode-Aspect (TMA)

| Marqueur | Fonction | Fréquence | Collocations principales |
|----------|----------|-----------|--------------------------|
| **ka** | Aspect progressif/habituel | 1,800 | fè, di, pran |
| **té** | Passé | 726 | ka, ké, ni |
| **ké** | Futur | 337 | pé, vlé |
| **kay** | Futur | 31 | — |
| **pa** | Négation | 664 | ka, té, ni |
| **ja** | Déjà (accompli) | 97 | — |

### 5.3 Exemples de Prédictions Contextuelles

| Mot source | Prédictions (probabilité) |
|------------|---------------------------|
| **ka** | fè (0.09), di (0.04), pran (0.03), vwè (0.03), palé (0.02) |
| **nou** | ka (0.12), ké (0.06), té (0.05), pa (0.05), pé (0.04) |
| **mwen** | ka (0.07), sé (0.05), té (0.05), an (0.05), pa (0.04) |
| **yo** | ka (0.18), té (0.08), pa (0.05), pé (0.02), sé (0.02) |
| **an** | ka (0.10), mwen (0.08), té (0.07), nou (0.06), pa (0.03) |
| **la** | ka (0.11), sé (0.06), ki (0.06), an (0.04), té (0.04) |
| **té** | ka (0.25), ké (0.15), ni (0.07), ja (0.03), pé (0.02) |
| **pa** | ka (0.13), té (0.10), ni (0.10), sav (0.02), janmen (0.02) |
| **tout** | sé (0.10), moun (0.07), biten (0.03), dòt (0.03), bèl (0.02) |
| **pou** | nou (0.07), yo (0.05), sa (0.05), fè (0.03), mwen (0.02) |

## 6. Mots Longs et Complexité Morphologique

### 6.1 Mots de 10 Lettres et Plus (572 mots)

| Rang | Mot | Longueur | Fréquence |
|------|-----|----------|-----------|
|  1 | **diz-nèf-san-swasannsèt** | 22 lettres | 1 |
|  2 | **sèvis-ladministrasyon** | 21 lettres | 1 |
|  3 | **rèsponsabilité-lasa** | 19 lettres | 1 |
|  4 | **lagyè-lendépandans** | 18 lettres | 1 |
|  5 | **katrè-d-laprémidi** | 17 lettres | 1 |
|  6 | **twazè-d-laprémidi** | 17 lettres | 1 |
|  7 | **propriyétè-fonsyé** | 17 lettres | 1 |
|  8 | **konkou-agrégasyon** | 17 lettres | 1 |
|  9 | **bazil-lanmòsibit** | 16 lettres | 1 |
| 10 | **lawa-a-gran-tété** | 16 lettres | 1 |
| 11 | **fanm-laplenn-lin** | 16 lettres | 1 |
| 12 | **pawòl-senk-é-kat** | 16 lettres | 1 |
| 13 | **konsititisyon-la** | 16 lettres | 1 |
| 14 | **toutmoun-bouliki** | 16 lettres | 1 |
| 15 | **kréyòl-gwadloup** | 15 lettres | 4 |
| 16 | **véridik-véritab** | 15 lettres | 2 |
| 17 | **noumenm-noumenm** | 15 lettres | 1 |
| 18 | **jiwomon-malanga** | 15 lettres | 1 |
| 19 | **charlottesville** | 15 lettres | 1 |
| 20 | **dèmen-spéra-dyé** | 15 lettres | 1 |
| 21 | **tout-sòt-kalité** | 15 lettres | 1 |
| 22 | **popilasyon-moun** | 15 lettres | 1 |
| 23 | **sitiyasyon-lasa** | 15 lettres | 1 |
| 24 | **konstitisyon-la** | 15 lettres | 1 |
| 25 | **pipirit-chantan** | 15 lettres | 1 |
| 26 | **pè-dè-bòn-famiy** | 15 lettres | 1 |
| 27 | **lapèldéchanpyon** | 15 lettres | 1 |
| 28 | **zè-d-laprémidi** | 14 lettres | 2 |
| 29 | **rèsponsabilité** | 14 lettres | 2 |
| 30 | **gratèdpapyé-la** | 14 lettres | 2 |

## 7. Évolution Diachronique du Lexique

- **Mots conservés** : 5,296 (100.0% de l'ancien dictionnaire)
- **Mots ajoutés** : 9
- **Mots supprimés** : 0

### 7.1 Nouveaux Mots Ajoutés (échantillon)

`cadina, joua, klavyé-la, kréyòl-la, makrèl, makrélaj, malmaké, tati, tonton`

## 8. Qualité et Validation Linguistique

### 8.1 Analyse de Qualité

- **Mots de 2 lettres** : 102
- **Mots avec chiffres** : 0
- **Cohérence orthographique** : ✓ Bonne

## 9. Métriques Linguistiques Avancées

- **Type-Token Ratio (TTR)** : 0.1452
- **Entropie lexicale (Shannon)** : 9.34 bits
- **Diversité lexicale** : Moyenne

## 10. Recommandations Linguistiques

### 10.1 Forces du Corpus

- Couverture lexicale importante (5,305 types)
- Richesse des bigrammes (20,307 patterns)
- Présence des marqueurs TMA caractéristiques du créole

### 10.2 Axes d'Amélioration

- Enrichir le vocabulaire technique et scientifique
- Documenter les variantes orthographiques
- Ajouter des métadonnées sémantiques (catégories grammaticales)
- Développer un système de lemmatisation

## Annexes

### A. Références Bibliographiques

- Bernabé, J. (1983). *Fondal-natal : Grammaire basilectale approchée des créoles guadeloupéen et martiniquais*.
- Ludwig, R., Montbrand, D., Poullet, H., & Telchid, S. (2001). *Dictionnaire créole-français (Guadeloupe)*.
- Hazaël-Massieux, M.-C. (2008). *Textes anciens en créole français de la Caraïbe*.

### B. Méthodologie

**Tokenisation** : Expression régulière Unicode préservant les diacritiques créoles

**N-grams** : Probabilités conditionnelles P(w₂|w₁) avec seuil de pertinence à 1%

**Normalisation** : Conversion en minuscules, préservation des traits d'union

---

*Rapport généré automatiquement par Kreyòl Potomitan™ Pipeline v3.0 - Pipeline Unique*

*Pou an kreyòl ki ka viv é ka evolyé !*
