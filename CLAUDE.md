...
### Writing aids (`AccentRestoration.kt`, `FieldKind.kt`, `InputProcessor.kt`)

Three aids act on what the user typed, and each has an off switch (`KeyboardPreferences`, « Écriture » in `KeyboardSettingsActivity`). They are computed once per focus into an `AidesEcriture` (`FieldKind` × the three switches × the editor's `NO_SUGGESTIONS` flag) and read by `InputProcessor` without recomputing on a keystroke.

- **Accent restoration** is the keyboard's only auto-correction, and it is bounded by design. It **never replaces a word by another**: input and output share their letters exactly, only accents are added (`AccentRestoration.choisir`, pure and covered by `AccentRestorationTest`, which also checks the property over the whole dictionary). It abstains for a word in the French lexicon (`se`, `le`, `de` are also accented Kréyòl words), for a word where the user typed any accent, for two rival accented spellings unless one leads by 5× (`pé` 244 vs `pè` 90: not arbitrated, the norm is for teachers to settle, see the orthographic-norm memory), and for an attested unaccented form unless the accented one leads by 8×. It runs at a space, at `, . ; : ? !` and at Enter, never at `-` or `'` (they extend a word). About 71 % of accented occurrences in the corpus are covered.
- **The correction is undoable**: a backspace right after it restores the typed word and the next validation of that word leaves it alone (`motRefuse`). The one-shot states (`derniereRestauration`, `espaceAutoEnAttente`, `instantDernierEspace`) are read then reset at the top of `processKeyPress` for every key, so each only concerns the key that follows the action that set it. `wordCommitListener` counts the *corrected* word; undoing does not uncount it (accepted: a rejected correction is rare and the count only feeds the discovery statistics).
- **Double space → period** needs both the 700 ms window and the text actually before the cursor (letter or digit, then one space): the clock alone would turn `bonjou, ` into `., `.
- **Punctuation drops the auto-space** only for `,` and `.`, and only the space the keyboard itself put after a suggestion. `?` and `!` keep their space on purpose: the corpus writes « Nou kay bengné ! » with French typography.
- **`FieldKind`** classifies `inputType` (the variation wins over the class, so a numeric PIN is a password). Number, phone and date open on the 123 panel via `InputProcessor.setNumericMode` (also called for other fields to put the processor back in step with the layout manager, which `forceAlphabeticMode()` resets). Email and URI swap the `é`/`è` slots of the bottom row for `@`/`_` or `/`/`_`; the panels are prebuilt and hidden, so the service rebuilds the cached input view when this changes (`dispositionDeLaVue`), like a theme change. Suggestions and the word history are off for every field but text and person names.
...
**Storage and the privacy boundary.** The collection is a `SharedPreferences` blob (`kreyol_carnet_prefs`). **Nothing goes to the cloud**: both backup rule files close the cloud backup by excluding every domain whole (a section with no `include` backs up *everything*, so an empty section would not do), and `BackupRulesTest` fails if an `include` comes back there. Only Android 12+'s `device-transfer` keeps the preferences, so the carnet follows the player to a new phone over cable or direct Wi-Fi, with no server in between; before Android 12 both are closed. Until 22.1.0 the rules included `path="."` in the cloud, contradicting the privacy policy, which promises nothing leaves the device. What enters the carnet is drawn from a closed shipped set — the same boundary as emoji recents, never free text. The counterpart is `PreuveDeFrappe`, "the keyboard is the exam": a card whose usage counter rose since the last session goes up a box with no question asked. Those counters (`creole_dict_with_usage.json`) and the reference it compares against (`carnet_vu.json`) live in `filesDir`, and both backup rule files **exclude them explicitly** in the device transfer, where the preferences do travel. Only a due date reaches the preferences, where it is indistinguishable from one pushed by a successful review. Do not "simplify" that reference into the preferences: it would ship a typing history to the cloud.
...
Since the fork's 26.0.1 to 26.2.0 (ported after 22.1.0) a simple vibrator (Galaxy A21s: it answers « unknown » to the amplitude query, not « no ») gets a 40 ms impulse instead of silence, the relief also reads vertically, and the word plate carries rivets, an engraved word (`MotGrave.kt`) and, from the higher tiers, a corner ironwork. The `carnet/` files were merged three-way from the fork's tree at 26.0.0, so they can still be re-synced the same way.
