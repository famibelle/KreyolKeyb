...
class SettingsActivity : AppCompatActivity() {
    ...
    companion object {
        ...
        private val WEEKLY_TIPS = listOf(
            ...
            "« Mots Mélangés », dans l'onglet « Jé », vous donne 10 mots à remettre dans l'ordre contre la montre, avec un bouton « Indice » quand vous bloquez.",
            ...
            "Les propositions sont rangées de gauche à droite : la première est celle que le clavier juge la plus probable."
        )
    }
    ...
    // tunnel, sans jeux ni statistiques, alors qu'il connaît déjà
    // l'application. Or ces onglets ne dépendent que des actifs livrés et
    // marchent sans clavier installé. La barre revient donc dès l'activation,
    // et ne repart plus.
    ...
    private fun createTabBar(): LinearLayout {
        return LinearLayout(this).apply {
            ...
            val gamesTab = createTab(TAB_JEUX, "🎮", "Jé")
    ...
    private fun createTab(tabIndex: Int, emoji: String, label: String): LinearLayout {
        ...
        return LinearLayout(this).apply {
            ...
            setOnClickListener { allerAOnglet(tabIndex) }
        }
    }
    ...
    fun createOnboardingContent(): LinearLayout {
        ...
        // qu'on lit une fois, et elles y coûtaient deux septièmes de la largeur
     ...
     *
     * Le défilement n'est animé que d'un onglet à son voisin : au-delà,
     * ViewPager2 s'arrête en chemin, et la barre affichait l'onglet demandé
     * pendant que le contenu était celui d'un autre.
     */
    ...
    private fun createQuickSetupCard(
        ...
        val sousTitre = TextView(this).apply {
            text = when {
                ...
                hasCompletedBefore && isEnabled -> "Le clavier kréyòl n'est plus celui qui s'affiche quand vous écrivez."
                hasCompletedBefore -> "Le clavier kréyòl n'est plus actif sur ce téléphone."
    ...
    fun createGuideContent(): LinearLayout {
        ...
        addGuideSection(
            ...
            "Cinq jeux (« Mots Mêlés », « Mots Mélangés », « Mo an Karénaj », " +
                    "« Fraz a twou » et « Mokwaré ») sont réunis sous l'onglet « Jé ». Ils " +
                    "font revoir du vocabulaire créole en s'amusant, à partir des mots déjà " +
                    "présents dans le dictionnaire du clavier."
        )
    ...
    private fun createRetourClavierCard(): LinearLayout {
        ...
        val titre = TextView(this).apply {
            text = "⌨️ Revenir au clavier kréyòl"
            textSize = 19f
        ...
        val explication = TextView(this).apply {
            text = "En ce moment, c'est un autre clavier qui s'ouvre quand vous écrivez. " +
                    "Le clavier kréyòl est toujours installé sur votre téléphone : " +
        ...
        val bouton = Button(this).apply {
            text = "Choisir le clavier kréyòl"
            textSize = 17f
            // Le thème AppCompat met les Button en capitales dans un contexte
            // d'activité : les autres appels à l'action de l'onglet n'en sont
            // pas, celui-ci n'a pas à détonner.
            isAllCaps = false
            setBackgroundColor(Color.parseColor("#0080FF"))
            setTextColor(Color.WHITE)
            minHeight = enDp(56)
    ...
    class WordSearchFragment : Fragment() {
        ...
        /** Ce que la grille en cours a versé au carnet, et ce qui y était neuf. */
        ...
        private fun generateNewPuzzle() {
            try {
                ...
                // Une nouvelle grille repart d'un bilan vide : la pochette ne
                // doit montrer que ce que la partie qui vient de finir a donné.
                enleverPochette()
                gagnes.clear()
                neuves.clear()
         ...
         *
         * Le mot était donné d'avance ici : ce qui se gagne n'est pas son sens
         * mais sa graphie, repérée lettre à lettre. C'est tout de même une
         * rencontre, et le carnet les garde toutes. La forme versée est celle
         * du dictionnaire, pas la capitale de la grille.
         */
        private fun encarter(forme: String?) {
            val ctx = context ?: return
            if (forme.isNullOrBlank()) return
            if (Carnet.ajouter(ctx, forme, JeuCarte.MOTS_MELES)) neuves.add(forme)
            gagnes.add(forme)
        }

        /** La pochette, une fois la grille complète. */
        private fun ouvrirPochette() {
            val grille = currentPuzzle
            enleverPochette()
            Pochette.ouvrir(
                fragment = this,
                jeu = JeuCarte.MOTS_MELES,
    ...
    class WordScrambleFragment : Fragment() {
        ...
        /** Ce que la manche a versé au carnet, et ce qui y était neuf. */
        private val gagnes = mutableListOf<String>()
        private val neuves = mutableSetOf<String>()
        private var pochette: View? = null
        ...
        /**
         * La fin de manche : les cartes d'abord, le score ensuite.
         *
        ...
        private fun encarter(forme: String) {
            ...
            if (Carnet.ajouter(ctx, forme, JeuCarte.MOTS_MELANGES)) neuves.add(forme)
            gagnes.add(forme)
        }
    ...
    class MoKarenajFragment : Fragment() {
         ...
         * Un mot perdu a été montré, pas deviné : il donne son sens — ce que le
         * jeu enseigne — mais pas sa carte. C'est la même règle que la
         * « Solution » de Mokwaré.
         */
        private fun endGame(won: Boolean) {
            ...
            val mot = targetWord
            val neuve = won && Carnet.ajouter(requireContext(), mot, JeuCarte.MO_AN_KARENAJ)

            enleverPochette()
            Pochette.ouvrir(
                fragment = this,
                jeu = JeuCarte.MO_AN_KARENAJ,
                formes = if (won) listOf(mot) else emptyList(),
                neuves = if (neuve) setOf(mot) else emptySet(),
                encoreValide = { targetWord == mot },
                surVue = { pochette = it },
                surFin = { if (isAdded && targetWord == mot) montrerLeBilan(won, mot) }
            )
        }

        private fun montrerLeBilan(won: Boolean, mot: String) {
            // La partie se termine sur le mot ; c'est le seul moment où sa
            // traduction ne gâche rien, et le seul où elle sert à quelque
            // chose.
            val glose = TranslationDictionary.libelle(
                requireContext(), mot, prefixe = "\n\nEn français : "
            )

            AlertDialog.Builder(requireContext())
                .setTitle(if (won) "🎉 Bravo !" else "😔 Domaj !")
                .setMessage(
                    (if (won) "Ou touvé mo-a an $currentAttempt èsèy : ${mot.uppercase()}"
                    else "Mo la té : ${mot.uppercase()}") + glose
                )
    ...
    class ClozeFragment : Fragment() {
        ...
        private val couleurNeutre = Color.parseColor("#FF8C00")
            ...
            rootView = ScrollView(activity).apply {
                ...
                val mainLayout = LinearLayout(activity).apply {
                    ...
                    val sentenceCard = LinearLayout(activity).apply {
                        ...
                        // La source est affichée phrase par phrase : ce sont des
                        // extraits d'œuvres d'auteurs identifiés, pas du texte
                        // anonyme.
                    ...
                    // Les quatre propositions, une par ligne : deux colonnes
                    // couperaient les formes agglutinées comme « an-mwen ».
                    ...
                    val rulesCard = LinearLayout(activity).apply {
                        ...
                        val rulesText = TextView(activity).apply {
                            text = "Chaque phrase est une phrase kréyòl réelle, à laquelle " +
                                "il manque un mot. Parmi les quatre propositions, une seule " +
                                "est celle qu'a écrite l'auteur : les trois autres sont des " +
                                "mots que le corpus atteste au même endroit, elles sonnent " +
                                "donc juste tant qu'on ne lit pas toute la phrase.\n\n" +
                        ...
                        val creditsText = TextView(activity).apply {
                            ...
                            text = "Phrases extraites du corpus :\n" +
        ...
        /**
         * La fin de manche : les cartes d'abord, le bilan ensuite.
         *
         * Une réponse trouvée verse sa carte, une réponse manquée non : la
         * bonne proposition est alors affichée en vert, donc montrée et pas
         * devinée, exactement comme le mot perdu de Mo an Karénaj.
         */
        ...
        /** Verse au carnet une réponse **trouvée**. */
        private fun encarter(forme: String) {
            val ctx = context ?: return
            if (forme.isBlank()) return
            if (Carnet.ajouter(ctx, forme, JeuCarte.FRAZ_A_TWOU)) neuves.add(forme)
            gagnes.add(forme)
        }
    ...
    /**
     * Fragment « Mokwaré » : une grille de mots croisés numérotée, ses
    ...
    class CrosswordFragment : Fragment() {
        ...
        /** Emplacements dont la carte était neuve, pour le bilan de la pochette. */
        ...
        override fun onCreateView(
            ...
            val defilement = ScrollView(activity).apply {
                ...
                val colonne = LinearLayout(activity).apply {
                    ...
                    // à la ScrollView parente, même en lecture seule.
                    ...
                    val carteRegles = LinearLayout(activity).apply {
                        ...
                        addView(TextView(activity).apply {
                            text = "Chaque définition est le sens français d'un mot " +
                                ...
                                "orthographe et son sens.\n\n" +
                                "La difficulté porte sur la taille de la grille et le " +
                                "nombre de mots."
                            textSize = 14f
                            setLineSpacing(0f, 1.2f)
                            setTextColor(Color.parseColor("#333333"))
        ...
        private fun nouvelleGrille() {
            ...
            if (grille == null) {
                ...
                tvDefinition.text = "Aucune grille disponible : l'actif " +
                    "creole_crossword.json manque à l'application."
                return
            }
         ...
         * colonnes : une taille fixe déborderait sur les grandes grilles, et
         * laisserait la moitié de l'écran vide sur les petites.
         */
         ...
         * rappelle la forme du dictionnaire et le sens : un mémo de vocabulaire,
         * puisque c'est ce qu'apprend le jeu.
         */
        private fun apresSaisie() {
            ...
            when {
                ...
                nouveaux.isNotEmpty() -> {
                    ...
                    tvRetour.text = "✅ ${mot.canonical} : ${mot.clue}"
                    tvRetour.setTextColor(couleurJuste)
                    tvRetour.visibility = View.VISIBLE
                }
                else -> {
         ...
         *
         * Mokwaré est le seul jeu où le joueur **écrit** le mot lui-même, à
         ...
         * capitales, et ni les accents ni la casse du dictionnaire n'y
         * survivent.
         *
         * Une grille révélée ne verse rien : `reveler()` remplit les cases sans
         * que personne les ait trouvées.
         */
        private fun encarter(nouveaux: List<Int>) {
            if (nouveaux.isEmpty() || solutionAffichee) return
            val ctx = context ?: return
            val grille = session?.grid ?: return
            nouveaux.forEach { emplacement ->
                val forme = grille.words[emplacement].canonical
                if (Carnet.ajouter(ctx, forme, JeuCarte.MOKWARE)) {
    ...
    /**
     * Onglet « Jé » : le choix des jeux, puis le jeu choisi.
     *
     * Ce fragment ne joue à rien lui-même. Il montre une carte par jeu et, au
     * tap, installe le fragment du jeu dans son propre conteneur. Aucun pager
     * imbriqué : les jeux comportent des grilles qui se manipulent au doigt
     * (le glissé des Mots Mêlés, notamment), et un second ViewPager leur aurait
     * disputé chaque geste horizontal.
     *
     * Le retour au choix passe par [androidx.activity.OnBackPressedCallback]
     * plutôt que par l'override d'`onBackPressed` de l'activité : le rappel
     * n'est actif que pendant qu'un jeu est ouvert, si bien que le bouton
     * Retour continue de quitter l'application partout ailleurs, sans que
     * l'activité ait à savoir ce que ses onglets contiennent.
     */
    class GamesFragment : Fragment() {
        ...
        private val jeux = listOf(
            // La révision en tête : c'est la seule carte qui ait quelque chose
            // à faire aujourd'hui plutôt qu'à n'importe quel moment. Le
            // septième jeu annoncé étant arrivé, la cale de la rangée impaire
            // ressert : sept cartes laissent la dernière rangée à moitié vide.
            Jeu("📚", "Sonjé", "Révisez vos cartes à intervalle régulier",
                "#5E35B1") { BoiteFragment() },
            Jeu("🎲", "Mots Mêlés", "Retrouvez les mots cachés dans la grille",
                "#9C27B0") { WordSearchFragment() },
            Jeu("🔤", "Mots Mélangés", "Remettez les lettres dans l'ordre",
            ...
            Jeu("🧩", "Mokwaré", "Écrivez les mots dans la grille, d'après leur sens",
                "#C2185B") { CrosswordFragment() },
            Jeu("🔡", "Mo an plas", "Casez les mots donnés dans la grille vide",
        ...
        /**
         * Charge la table de gloses et les phrases du carnet, en fond, dès que
         * l'onglet s'ouvre.
         *
         * Depuis la 21.0.0 tout mot gagné passe par `Carnet.ajouter`, qui
         * refuse ce qui n'a pas de glose : la table est donc sur le chemin
         * d'une bonne réponse dans les cinq jeux. Mokwaré est le cas qui
         * l'impose — c'est le seul jeu qui ne consultait jamais les gloses, et
         * sans ce préchargement le premier mot trouvé ferait analyser 1,2 Mo de
         * JSON sur le fil principal, au moment précis où la grille s'anime.
         *
         * Les deux chargements sont idempotents et synchronisés : l'appeler à
         * chaque ouverture de l'onglet ne coûte rien après la première fois.
         */
        private fun prechargerLesGloses(activity: SettingsActivity) {
        ...
        private fun construireGrilleChoix(activity: SettingsActivity): View {
            ...
            // Deux cartes par ligne : une carte pleine largeur pour chacun
            // aurait poussé les derniers jeux hors de l'écran, là où on ne les
            // découvre plus. Une ligne incomplète (le nombre de jeux est
            // impair) reçoit une cale de même poids, faute de quoi sa carte
            ...
            // La bannière ajoute une ligne au-dessus des cartes : sans
            // défilement, la dernière rangée de jeux tombe sous le bord sur un
            // écran court, et rien ne le signalerait. Le poids fait prendre à
            // la vue la hauteur restante et jamais plus, sinon les cartes se
        ...
        /**
         * La bannière du carnet, au-dessus des cinq jeux.
         *
         * Le carnet est ce qui relie les cinq parties entre elles, et la seule
         * chose qui reste quand la partie est finie : il est donc en tête,
         * pleine largeur, au-dessus des jeux plutôt qu'à côté d'eux. Le mettre
         * au fond d'un jeu obligerait à en avoir fini une partie pour le
         * découvrir.
         *
         ...
         * demanderaient le balayage de `creole_dict.json`, qui n'a rien à faire
         * sur le fil principal à l'ouverture d'un onglet — elles sont dans le
         * carnet lui-même, à une touche d'ici.
         */
         ...
         *
         * Appelée à la construction et à chaque retour sur le menu : une partie
    ...
    class ChasseCroiseFragment : Fragment() {
        ...
        override fun onCreateView(
            ...
            rootView = ScrollView(activity).apply {
                ...
                val colonne = LinearLayout(activity).apply {
                    ...
                    // même en lecture seule (même piège que Mokwaré).
                    conteneurGrille = LinearLayout(activity).apply {
                        layoutParams = LinearLayout.LayoutParams(
                            LinearLayout.LayoutParams.WRAP_CONTENT,
                            LinearLayout.LayoutParams.WRAP_CONTENT
                    ...
                    val carteRegles = LinearLayout(activity).apply {
                        ...
                        addView(TextView(activity).apply {
                            text = "Tous les mots vous sont donnés : il s'agit de " +
                                ...
                                "Aucune connaissance du kréyòl n'est " +
         ...
         *
         * Le rappel s'arrête là. La forme canonique porte les accents que la
         * grille perd, et c'est tout ce qu'elle apprend : le kréyòl ne met de
         * majuscule qu'aux noms propres, que le vivier écarte, donc il n'y a
         * pas de casse à rappeler — le fork luxembourgeois, lui, doit le faire
         * pour ses substantifs.
         */
        ...
        private fun encarter(nouveaux: List<Int>) {
            ...
            nouveaux.forEach { emplacement ->
                val forme = grille.words[emplacement].canonical
                if (Carnet.ajouter(ctx, forme, JeuCarte.MO_AN_PLAS)) {
                    cartesNeuves.add(emplacement)
                }
            }
            majBoutonCarnet()
     ...
     *
     * Un [androidx.fragment.app.DialogFragment] plutôt qu'une Activity : les
     * deux pages existent déjà sous forme de Fragment, et les héberger ici
     * évite deux déclarations de manifeste et deux cycles de vie de plus pour
     * un contenu qu'on ouvre et qu'on referme.
     */
    class SheetFragment : androidx.fragment.app.DialogFragment() {
        ...
        companion object {
            ...
            fun pour(page: String) = SheetFragment().apply {
                arguments = Bundle().apply { putString(ARG_PAGE, page) }
            }
        }
